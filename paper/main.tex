...
The 100K results in the main paper use a 1,000-step warmup, while the official Moirai~2.0 training protocol~\citep{liu2024moirai2} uses 10,000 steps of warmup. To control for this protocol difference, we retrain the baseline at 100K steps with 10K warmup. The baseline with 10K warmup achieves 0.893 normalized MASE (3-seed mean: s0=0.882, s2=0.880, s7=0.917), comparable to the 1K-warmup baseline (0.878), confirming that the warmup length has minimal effect on baseline performance.

\emph{Note:} The matched-official HD10 runs (with 10K warmup at 100K steps) are currently being re-run with corrected configuration. Results will be updated upon completion.
