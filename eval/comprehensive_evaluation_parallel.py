#!/usr/bin/env python3
"""
Parallel Comprehensive Model Evaluation Script

This is a parallelized version of comprehensive_evaluation.py that uses multiprocessing
to run evaluations on multiple datasets simultaneously, speeding up evaluation on GPU nodes.

Usage Examples:

    # Standard evaluation with 4 parallel workers
    python comprehensive_evaluation_parallel.py --mode standard --model-version 1.1 --num-workers 4

    # Preconditioned space evaluation with 8 parallel workers
    python comprehensive_evaluation_parallel.py --mode precond \
        --model-path /path/to/precond_checkpoint.ckpt \
        --precond-type chebyshev --precond-degree 5 \
        --num-workers 8

    # Baseline in preconditioned space with auto-detection of CPU count
    python comprehensive_evaluation_parallel.py --mode baseline-precond \
        --model-path /path/to/baseline_checkpoint.ckpt \
        --precond-type chebyshev --precond-degree 5 \
        --num-workers auto

Key Improvements:
- Multiprocessing for parallel dataset evaluation
- Configurable number of workers (--num-workers)
...
def evaluate_single_dataset_standard(
    args_tuple: Tuple[int, int, Dict, str, str, int, int, int, Path, Optional[str], dict]
) -> Dict:
    """
    Worker function to evaluate a single dataset (standard mode).

    Args:
        args_tuple: Tuple containing (idx, total, dataset, model_config, model_name,
                    patch_size, context_length, batch_size, output_dir, model_path, progress_dict)

    Returns:
        Dictionary with evaluation metrics
    """
    idx, total, dataset, model_config, model_name, patch_size, context_length, batch_size, output_dir, model_path, progress_dict = args_tuple
    ...
    # Build command
    model_type = 'custom' if model_path else 'official'
    cmd = [
        'python', '-m', 'cli.eval',
    ...
    if progress_dict is not None:
        ...
        status_symbol = "✓" if (exit_code == 0 and metrics.get('status') == 'success') else "✗"
        print(f"  [{completed}/{total}] {status_symbol} {display_name}")

    return metrics


def evaluate_single_dataset_precond(
    args_tuple: Tuple[int, int, Dict, str, str, int, int, int, Path, str, str, int, dict]
) -> Dict:
    """
    Worker function to evaluate a single dataset (precond mode).

    Args:
        args_tuple: Tuple containing (idx, total, dataset, model_path, model_name,
                    patch_size, context_length, batch_size, output_dir,
                    precond_type, precond_degree, progress_dict)

    Returns:
        Dictionary with evaluation metrics
    """
    idx, total, dataset, model_path, model_name, patch_size, context_length, batch_size, output_dir, precond_type, precond_degree, progress_dict = args_tuple
...
def evaluate_single_dataset_baseline_precond(
    args_tuple: Tuple[int, int, Dict, str, str, int, int, int, Path, str, int, dict]
) -> Dict:
    """
    Worker function to evaluate a single dataset (baseline-precond mode).
...
# ============================================================================
# Evaluation Mode 1: Standard Evaluation (with reversal) - Parallel
# ============================================================================

def eval_standard_parallel(
    ...
    """
    Standard evaluation with parallel processing (reproduces eval_comprehensive.slurm).
    ...
    """
    print("=" * 80)
    print("Standard Evaluation (Original Space) - PARALLEL MODE")
    print("=" * 80)
    ...
    print(f"Starting parallel evaluation of {total} datasets...")
    print()
    ...
    args_list = [
        (idx, total, dataset, model_config, model_name, patch_size, context_length,
         batch_size, output_dir, model_path, progress_dict)
        for idx, dataset in enumerate(eval_datasets, 1)
    ]

    # Run parallel evaluation
    with Pool(processes=num_workers) as pool:
        results = pool.map(evaluate_single_dataset_standard, args_list)
...
# ============================================================================
# Evaluation Mode 2: Preconditioned Space Evaluation (no reversal) - Parallel
# ============================================================================

def eval_precond_space_parallel(
    model_path: str,
    ...
    """
    Preconditioned space evaluation with parallel processing (reproduces eval_precond_comprehensive.slurm).
    ...
    """
    print("=" * 80)
    print("Preconditioned Space Evaluation (No Reversal) - PARALLEL MODE")
    print("=" * 80)
    ...
    # Prepare arguments for parallel processing
    args_list = [
        (idx, total, dataset, model_path, model_name, patch_size, context_length,
         batch_size, output_dir, precond_type, precond_degree, progress_dict)
        for idx, dataset in enumerate(eval_datasets, 1)
    ]

    # Run parallel evaluation
    with Pool(processes=num_workers) as pool:
        results = pool.map(evaluate_single_dataset_precond, args_list)
...
# ============================================================================
# Evaluation Mode 3: Baseline in Preconditioned Space - Parallel
# ============================================================================

def eval_baseline_in_precond_space_parallel(
    model_path: str,
    precond_type: str = 'chebyshev',
    precond_degree: int = 5,
    patch_size: int = 32,
    ...
    """
    Baseline model in preconditioned space evaluation with parallel processing
    (reproduces eval_baseline_in_precond_space.slurm).
    ...
    """
    print("=" * 80)
    print("Baseline Model in Preconditioned Space Evaluation - PARALLEL MODE")
    print("=" * 80)
    ...
    # Run parallel evaluation
    with Pool(processes=num_workers) as pool:
        results = pool.map(evaluate_single_dataset_baseline_precond, args_list)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Parallel Comprehensive Model Evaluation Script',
    ...
    parser.add_argument('--num-workers', type=str, default='4',
                        help='Number of parallel workers (integer or "auto" for CPU count)')
...
def main():
    ...
    if args.mode == 'standard':
        print("Running standard evaluation (parallel)...")
        eval_standard_parallel(
        ...
        print("Running preconditioned space evaluation (parallel)...")
        eval_precond_space_parallel(
            model_path=args.model_path,
        ...
        print("Running baseline in preconditioned space evaluation (parallel)...")
        eval_baseline_in_precond_space_parallel(
            model_path=args.model_path,
            precond_type=args.precond_type,
            precond_degree=args.precond_degree,
            patch_size=args.patch_size,
...
def new_main():
    ...
    df = eval_precond_space_parallel(
        model_path=our_precond_model_path,
        output_dir="eval-runs/precond_space_parallel",
        num_workers=8
    )
    df = eval_baseline_in_precond_space_parallel(
        model_path=our_pretrained_model_path,
        output_dir="eval-runs/baseline_in_precond_space_parallel",
        num_workers=8
    )
